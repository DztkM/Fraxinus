...
async def test_init_upload(client, mock_s3):
    ...
    response = await client.post("/api/files/upload/init", json=request_data)
...
async def test_complete_upload(client, mock_s3):
    ...
    complete_resp = await client.post(f"/api/files/{file_id}/upload/complete", json=complete_data)
...
async def test_download_file(client, mock_s3):
    # 1. Init
    init_resp = await client.post("/api/files/upload/init", json={
    ...
    # 2. Complete
    await client.post(f"/api/files/{file_id}/upload/complete", json={
        "upload_id": "test_upload_id",
        "parts": [{"PartNumber": 1, "ETag": "etag"}]
    })
    
    # 3. Download
    download_resp = await client.get(f"/api/files/{file_id}/download")
...
async def test_list_files(client, mock_s3):
    await client.post("/api/files/upload/init", json={
...
async def test_update_file(client, mock_s3):
    ...
    update_resp = await client.patch(f"/api/files/{file_id}", json={"original_name": "new_name.txt"})
...
async def test_access_deleted_file(client, mock_s3):
    ...
    # 2. Delete file
    await client.delete(f"/api/files/{file_id}")
