...
async def test_update_folder_access(client):
    # 1. Create folder
    folder_resp = await client.post("/api/folders/", json={"name": "Access Folder"})
    ...
    patch_resp = await client.patch(f"/api/folders/{folder_id}/access", json=patch_data)
...
async def test_folder_access_invalid_level(client):
    folder_resp = await client.post("/api/folders/", json={"name": "Access Folder 2"})
    folder_id = folder_resp.json()["id"]
...
async def test_file_access_propagation(client, mock_s3, db_session):
    ...
    parent_resp = await client.post("/api/folders/", json={"name": "Parent Folder"})
    ...
    init_resp = await client.post("/api/files/upload/init", json=init_data)
    file_id = init_resp.json()["file_id"]
    
    # 3. Update parent folder access to 3
    patch_resp = await client.patch(f"/api/folders/{parent_id}/access", json={"set_access_level": 3})
    assert patch_resp.status_code == 200
...
async def test_shared_endpoint(client):
    # 1. Create folder
    folder_resp = await client.post("/api/folders/", json={"name": "Shared Folder"})
    folder_id = folder_resp.json()["id"]
    ...
    await client.patch(f"/api/folders/{folder_id}/access", json=patch_data)
    
    # 3. Get shared items
    shared_resp = await client.get("/api/shared/")
...
async def test_file_access_propagation_deep_higher(client, mock_s3, db_session):
    root_resp = await client.post("/api/folders/", json={"name": "Root"})
    root_id = root_resp.json()["id"]

    sub1_resp = await client.post("/api/folders/", json={"name": "Sub 1", "parent_id": root_id})
    sub1_id = sub1_resp.json()["id"]

    sub2_resp = await client.post("/api/folders/", json={"name": "Sub 2", "parent_id": root_id})
    sub2_id = sub2_resp.json()["id"]
    await client.patch(f"/api/folders/{sub2_id}/access", json={"set_access_level": 3})
    await recalculate_folder_access(uuid.UUID(sub2_id), db_session)

    f1_resp = await client.post("/api/files/upload/init", json={"original_name": "f1.txt", "size": 1, "mime_type": "text/plain", "parts_count": 1, "folder_id": sub1_id})
    f1_id = f1_resp.json()["file_id"]

    f2_resp = await client.post("/api/files/upload/init", json={"original_name": "f2.txt", "size": 1, "mime_type": "text/plain", "parts_count": 1, "folder_id": sub2_id})
    f2_id = f2_resp.json()["file_id"]

    await client.patch(f"/api/folders/{root_id}/access", json={"set_access_level": 2, "allowed_users": ["u1"]})
    await recalculate_folder_access(uuid.UUID(root_id), db_session)

    sub1 = await client.get(f"/api/folders/{root_id}/contents")
    ...
    f1_contents = await client.get(f"/api/folders/{sub1_id}/contents")
    file1 = next(f for f in f1_contents.json()["files"] if f["id"] == f1_id)
    assert file1["actual_access_level"] == 2

    f2_contents = await client.get(f"/api/folders/{sub2_id}/contents")
...
async def test_file_access_propagation_deep_lower(client, mock_s3, db_session):
    root_resp = await client.post("/api/folders/", json={"name": "Root"})
    root_id = root_resp.json()["id"]
    await client.patch(f"/api/folders/{root_id}/access", json={"set_access_level": 3})
    await recalculate_folder_access(uuid.UUID(root_id), db_session)

    sub1_resp = await client.post("/api/folders/", json={"name": "Sub 1", "parent_id": root_id})
    sub1_id = sub1_resp.json()["id"]
    await client.patch(f"/api/folders/{sub1_id}/access", json={"set_access_level": 2, "allowed_users": ["u1"]})
    ...
    await client.patch(f"/api/folders/{root_id}/access", json={"set_access_level": 1})
    await recalculate_folder_access(uuid.UUID(root_id), db_session)

    root_contents = await client.get(f"/api/folders/{root_id}/contents")
